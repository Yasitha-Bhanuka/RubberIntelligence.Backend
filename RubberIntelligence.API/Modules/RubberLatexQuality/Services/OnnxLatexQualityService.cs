    ...
    public class OnnxLatexQualityService : ILatexQualityService
    {
        ...
        public async Task<LatexQualityResponse> PredictQualityAsync(LatexQualityRequest request)
        {
            ...
            try
            {
                ...
                var (grade, status) = GetQualityGradeAndStatus(qualityScore);
                
                // Generate recommendations
                var recommendations = GenerateRecommendations(request, qualityScore);

                _logger.LogInformation($"[LatexQualityAI] Prediction: {grade} (Score: {qualityScore}, Confidence: {confidence:P2})");
        ...
        private (string Grade, string Status) GetQualityGradeAndStatus(int score)
        {
            if (score >= 70)
                return ("Good", "Pass");
            else if (score >= 50)
        ...
        private string[] GenerateRecommendations(LatexQualityRequest request, int qualityScore)
        {
            ...
            {
                recommendations.Add("Poor quality latex. Minor adjustments may improve consistency.");
            }
            else if (qualityScore >= 30)
            {
                recommendations.Add("Quality is below acceptable standards. Immediate corrective action required.");
                recommendations.Add("Consult with quality control team to identify root causes.");
            }
            else
            {
                recommendations.Add("Quality is very below acceptable standards. rubber latex sample is not suitable for processing.");
            }
